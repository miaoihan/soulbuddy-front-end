...
<!-- 咨询师详情 -->
  <div>
	  				...
	  				<div class="z-desc">
				  		<div class="desc-l1">
				  			<span class="d-l1-name" 
				  						:style="data.role == 0? 'color:#fff' : 'color:#444' ">
				  						{{data.name}}</span>
				  			<!-- 认证标志 -->
				  			<span>
				  				<i></i>
				  		...
				  		<div class="desc-l3" style="margin-top: 0.40rem">
				  			<span class="label" 
											:class="data.role==0? 'label-border': 'label-grey' "
				  						style="margin-right: 0.5rem" 
				  						v-for="it in data.label">
				  				{{it.name}}
				  			</span>
				  		</div>
				  	</div>
  	...
  	<div class="m-tips">
  			共回答了 <strong>{{ data.answerList.length }}</strong> 个问题</div>
		<div class="ans-list">
			<question-list></question-list>	
		</div>
		<footer class="qd-footer fixed-bottom ztc"
  					@click="ask">
  		￥{{ data.money }} 向{{ data.name }}提一个问题
  	</footer>
  </div>
</template>
